...
function safeCallback(callback) {return function () {safeCall(null, callback);};}
function safeTimeout(fn, time) {setTimeout(function () {safeCall(null, fn);}, time);}
function isDefined(x) {return "undefined" !== typeof x;}
...
var loginUrlBase = "https://www.playalterego.com/api/";
...
function login(email, password, register, subscribe, callback) {xhrAuthRequest("POST", "login", callback, "email", encodeURIComponent(email), "password", encodeURIComponent(password), "register", register, "subscribe", subscribe);}
function forgotPassword(email, callback) {xhrAuthRequest("POST", "forgot", callback, "email", encodeURIComponent(email));}
...
function getRemoteEmail(callback) {xhrAuthRequest("GET", "getuser", callback);}
...
function clearCookie(callback, slot) {writeCookie('', slot, safeCallback(callback));}
function areSaveSlotsSupported() {return !!(initStore() && window.Persist.type != "cookie");}
...
function getDirtySaveList(callback) {restoreObject(initStore(), "dirty_save_list", [], function (slotList) {callback(slotList);});}
...
function getSaves(callback) {
    if (window.remoteStoreNames && window.remoteStoreNames.length) {
        ...
        restoreObject(remoteStore, "save_list", [], function (remoteSlotList) {fetchSavesFromSlotList(remoteStore, remoteSlotList, 0, [], function (remoteSaveList) {mergeRemoteSaves(remoteSaveList, 0, callback);});});
    } else {
        restoreObject(initStore(), "save_list", [], function (localSlotList) {fetchSavesFromSlotList(initStore(), localSlotList, 0, [], callback);});
    }
}
function fetchSavesFromSlotList(store, slotList, i, saveList, callback) {
    ...
    restoreObject(store, "state" + slotList[i], null, function (saveState) {
        saveState.timestamp = slotList[i].substring(4);
...
webSaveDomain = "www.playalterego.com";
...
function submitRemoteSave(slot, email, subscribe, callback) {
    ...
    window.store.get("state" + slot, function (ok, value) {
        if (ok) {
            var timestamp = slot.substring(4);
            var xhr = findXhr();
            ...
            xhr.onreadystatechange = function () {
                ...
                if (ok) {
                    ...
                    recordDirtySlots([slot], function () {safeCall(null, function () {callback(false);});});
                }
            };
            xhr.send(params);
        } else {
            recordDirtySlots([slot], function () {asyncAlert("There was a problem uploading the saved game. This is probably a bug; please contact support@playalterego.com with code 17891.", function () {safeCall(null, function () {callback(false);});});});
        }
    });
}
function submitDirtySaves(dirtySaveList, email, callback) {
    function submitDirtySave(i) {
        if (dirtySaveList[i]) {
            ...
            window.store.remove("dirty_save_list", function () {safeCall(null, function () {callback(true);});});
        }
    }
...
function mergeRemoteSaves(remoteSaveList, recordDirty, callback) {
    ...
    restoreObject(initStore(), "save_list", [], function (localSlotList) {
        fetchSavesFromSlotList(initStore(), localSlotList, 0, [], function (localSaveList) {
            ...
            function finale() {
                if (newRemoteSaves) {
                    window.store.set("save_list", toJson(localSlotList), function () {safeCall(null, function () {callback(localSaveList, newRemoteSaves, dirtySaveList);});});
                } else {
...
function delayBreakEnd() {if (initStore())window.store.remove("delayBreakStart");}
function initStore() {
...
function upgradeAndroidCookies(slot, forcedScene) {
    ...
    function upgradeComponent(i) {
        ...
        window.store.set(key, value, function () {upgradeComponent(i + 1);});
    }
...
function restoreGame(state, forcedScene, userRestored) {
    ...
    if (!isStateValid(state)) {
        ...
        scene = new Scene(startupScene, window.stats, window.nav, {
        ...
        scene = new Scene(state.stats.sceneName, state.stats, window.nav, {
