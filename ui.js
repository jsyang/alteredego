...
function showMenu() {
    ...
    function menu() {
        options = [{
            ...
            name      : "Email us at " + getSupportEmail() + ".",
            group     : "choice",
            contactUs : true
        }, {
...
function printAchievements(target) {
    ...
    for (var i = 0; i < totalAchievements; i++) {
        ...
        buffer.push(":");
        buffer.push("</b> ");
        buffer.push(description.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/\[b\]/g, '<b>').replace(/\[\/b\]/g, '</b>').replace(/\[i\]/g, '<i>').replace(/\[\/i\]/g, '</i>'));
        buffer.push(" (");
        buffer.push(points);
        buffer.push(" points)");
    }
...
function isShareConfigured() {return !!document.getElementById("share");}
function shareAction(e) {
    clearScreen(function () {
        var target = document.getElementById('text');
        printShareLinks(target, "now");
        printButton("Next", target, false, function () {clearScreen(loadAndRestoreGame);});
    });
}
...
function subscribeByMail(target, now, callback, code) {
    if (now) {
        ...
        println("Click here to subscribe to the Choice of Games mailing list; they'll notify you when a new multiple-choice game is ready!");
        println("");
        printButton("Subscribe", target, false, function () {code();});
        printButton("Next", target, false, function () {safeTimeout(function () {callback(now);}, 0);});
        printFooter();
    }
}
function subscribe(target, now, callback) {
    if (!target)target = document.getElementById('text');
    if (window.isIosApp) {
        subscribeByMail(target, now, callback, function () {callIos("subscribe");});
        return;
    }
    ...
    if (mailToSupported) {
        subscribeByMail(target, now, callback, function () {window.location.href = "mailto:subscribe-" + window.storeName + "-" + platformCode() + "@choiceofgames.com?subject=Sign me up&body=Please notify me when the next game is ready.";});
        return;
    }
    println("Type your email address below to subscribe to the Choice of Games mailing list; they'll notify you when a new multiple-choice game is ready!");
    println("");
            ...
            window["jsonp" + timestamp] = function (response) {
                ...
                if (response.result == "error") {
                    ...
                    clearScreen(function () {
                        ...
                        println("", target);
                        printButton("Next", target, false, function () {safeCall(null, callback);});
                    });
                }
            };
                    ...
                    if (isXhrAllowed) {
                        ...
                        xhr.onreadystatechange = function () {
                            ...
                            if (xhr.status == 200) {
                                ...
                                window["jsonp" + timestamp]({
                                    result : "error",
                                    msg    : "There was a network error submitting your registration. Please try again later, or email subscribe@choiceofgames.com instead."
                                });
                            } else {
                                window["jsonp" + timestamp]({
                                    result : "error",
                                    msg    : "Sorry, our mail server had an error. It's our fault. Please try again later, or email subscribe@choiceofgames.com instead."
                                });
                            }
                        };
                        xhr.send();
            ...
            xhrAuthRequest("GET", "get-purchases", function (ok, response) {
                ...
                if (ok) {
                    ...
                    if (response.error != "not registered") {
                        alertify.error("There was an error downloading your purchases from Playalterego.com. " + "Please refresh this page to try again, or contact support@playalterego.com for assistance.", 15000);
                    }
                }
...
function isWebPurchaseSupported() {return window.isSecureWeb && isWebSavePossible() && window.stripeKey;}
function isRestorePurchasesSupported() {return !!window.isIosApp || isWebPurchaseSupported();}
...
function getPrice(product, callback) {
    if (window.isIosApp) {
        ...
        safeTimeout(function () {callback.call(this, "guess");}, 0);
    }
}
function purchase(product, callback) {
    ...
    if (window.isIosApp) {
        ...
        if (!window.StripeCheckout)return asyncAlert("Sorry, we weren't able to initiate payment. (Your " + "network connection may be down.) Please refresh the page and try again, or contact " + "support@playalterego.com for assistance.");
        startLoading();
        isRegistered(function (registered) {
            doneLoading();
            ...
            function stripe(email) {
                ...
                xhrAuthRequest("GET", "product-data", function (ok, data) {
                    doneLoading();
                    if (!ok)return asyncAlert("Sorry, we weren't able to initiate payment. (Your " + "network connection may be down.) Please refresh the page and try again, or contact " + "support@playalterego.com for assistance.");
                            ...
                            clearScreen(function () {
                                ...
                                xhrAuthRequest("POST", "purchase", function (ok, response) {
                                    ...
                                    if (ok) {
                                        ...
                                        asyncAlert("Sorry, there was an error handling your purchase. Please wait five minutes and try again, or contact support@playalterego.com for assistance.");
                                        clearScreen(loadAndRestoreGame);
                                    } else {
                                        asyncAlert("Sorry, there was an error processing your card. (Your " + "network connection may be down.) Please refresh the page and try again, or contact " + "support@playalterego.com for assistance.");
                                        clearScreen(loadAndRestoreGame);
                                    }
...
function isAdvertisingSupported() {return typeof window != "undefined" && (window.isIosApp || window.isAndroidApp);}
function isFullScreenAdvertisingSupported() {return window.isIosApp || window.isAndroidApp;}
...
function promptEmailAddress(target, defaultEmail, callback) {
    ...
    println("", form);
    printButton("Next", form, true);
    printButton("No, Thanks", target, false, function () {callback(true);});
    form.onsubmit = function (e) {
        preventDefault(e);
        safeCall(this, function () {
            ...
            if (!/^\S+@\S+\.\S+$/.test(email)) {
                ...
                recordEmail(email, function () {callback(false, email);});
            }
        });
    };
    curl();
    ...
    fetchEmail(function (defaultEmail) {
        isRegistered(function (registered) {
            ...
            form.onsubmit = function (event) {
                            ...
                            if (grantedEmail) {
                                xhrAuthRequest("POST", "facebook-login", function (ok, response) {
                                    if (ok) {
                                        ...
                                        asyncAlert("Sorry, we weren't able to sign you in with Facebook. (Your network connection may be down.) Please try again later, or contact support@playalterego.com for assistance.");
                                    }
                ...
                if ("google" == choice) {
                    if (!window.gapi)return asyncAlert("Sorry, we weren't able to sign you in with Google. (Your network connection may be down.) Please try again later, or contact support@playalterego.com for assistance.");
                    var done = false;
                                ...
                                isRegistered(function (registered) {
                                    if (!registered)xhrAuthRequest("POST", "google-login", function (ok, response) {
                                        ...
                                        if (ok) {
                                            ...
                                            asyncAlert("Sorry, we weren't able to sign you in with Google. Please try again later, or contact support@playalterego.com for assistance.");
                                        }
                    ...
                    recordEmail(email, function () {
                        if ("yes" == choice) {
                            ...
                            form.onsubmit = function (event) {
                                ...
                                window.scrollTo(0, 0);
                                login(email, form.password.value, true, subscribe, function (ok, response) {
                                    doneLoading();
                                    if (ok) {
                                        target.innerHTML = "";
                                        loginDiv(ok, email);
                                        ...
                                        form.style.display = "";
                                        showMessage("Sorry, we weren't able to sign you in. (Your network connection may be down.) Please try again later, or contact support@playalterego.com for assistance.");
                                    }
                                });
                            };
                            target.appendChild(form);
                            ...
                            printButton("Next", form, true);
                            if (optional) {
                                printButton("Cancel", form, false, function () {safeCall(null, function () {callback(false);});});
                            }
                        } else if ("passwordButton" == choice) {
                            startLoading();
                            form.style.display = "none";
                            window.scrollTo(0, 0);
                            login(email, form.password.value, false, form.subscribe.checked, function (ok, response) {
                                doneLoading();
                                form.style.display = "";
                                if (ok) {
                                    target.innerHTML = "";
...
function isRegisterAllowed() {return window.isWeb || window.isIosApp;}
...
function getPassword(target, code) {
    ...
    println("", target);
    printButton("Next", target, false, function () {code(false, textArea.value);});
    printButton("Cancel", target, false, function () {code(true);});
}
        ...
        if (ok)alertify.prompt("Please type your email address.", function (ok, email) {
            if (ok)xhrAuthRequest("POST", "support-mail", function (ok, response) {
                if (ok) {
                    ...
                    alertify.alert("Bug reporting failed. Please email your bug report to support@playalterego.com (and be sure to mention that the bug reporter failed!)");
                }
            }, "email", encodeURIComponent(email), "subject", encodeURIComponent("bug report " + window.storeName), "text", encodeURIComponent(body));
        });
    });
}
window.registered = false;
function getSupportEmail() {
    if (window.storeName) {
        return "support-" + storeName + "-" + platformCode() + "@playalterego.com";
    }
    try {
        return document.getElementById("supportEmail").getAttribute("href").substring(7);
    } catch (e) {
        return "support-external@playalterego.com";
    }
}
function absolutizeAboutLink() {
    ...
    if (aboutLink) {
        ...
        if (/^https?:/.test(canonical)) {
            ...
            absoluteCanonical = "https://www.playalterego.com" + canonical;
        } else {
            absoluteCanonical = "https://www.playalterego.com/" + canonical;
        }
        if (!/\/$/.test(canonical)) {
...
function aboutClick() {window.location.href = document.getElementById("aboutLink").href;}
window.onerror = function (msg, file, line, stack) {
    ...
    if (ok) {
        ...
        if (window.Persist)body += "\nPersist: " + window.Persist.type;
        body += "\n\n" + statMsg + "\n\nversion=" + window.version;
        var supportEmailHref = "mailto:support-external@playalterego.com";
        try {
...
window.onload = function () {
    ...
    isRegistered(function (registered) {
        if (registered) {
            fetchEmail(function (email) {loginDiv(registered, email);});
        } else {
            loginDiv();
        }
    });
    ...
    if (isFollowEnabled()) {
        ...
        if (shareElement)shareElement.innerHTML = '<iframe src="//www.facebook.com/plugins/like.php?href=https%3A%2F%2Fwww.facebook.com%2Fchoiceofgames&amp;send=false' + '&amp;layout=button_count&amp;width=90&amp;show_faces=true&amp;font&amp;colorscheme=light&amp;action=like&amp;height=20&amp;appId=190439350983878"' + ' scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:90px; height:20px;" allowTransparency="true"></iframe>' + '<iframe allowtransparency="true" frameborder="0" scrolling="no" ' + 'src="//platform.twitter.com/widgets/follow_button.html?screen_name=choiceofgames&amp;show_screen_name=false"' + ' style="width:150px; height:20px;"></iframe>';
    }
...
if (!isWeb && window.isIosApp) {
    ...
    document.write("<style>" + "#header { display: none; }" + "" + "#emailUs { display: none; }" + "" + "#main { padding-top: 1em; }" + "</style>");
}
...
if (window.isMacApp || window.isWinOldApp || window.isCef || window.isAndroidApp) {
    document.write("<style>" + "#headerLinks { display: none; }" + "" + "#emailUs { display: none; }" + "" + "</style>");
}
...
if (window.isChromeApp) {
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        if (statsButton) {
            ...
            statsButton.addEventListener("click", function () {showStats();}, false);
        }
...
if (window.isCef) {
    var pollPurchases = function () {
        cefQuery({
            ...
            onFailure : function (error_code, error_message) {console.error("PollPurchases error: " + error_message);}
        });
    };
    pollPurchases();
}
