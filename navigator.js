...
SceneNavigator.prototype.nextSceneName = function nextSceneName(currentSceneName) {
    if (currentSceneName == "navigator")return null;
    var intermission = /^(male|female)-inter(\d)/.exec(currentSceneName);
    if (intermission) {
        var gender = intermission[1];
        var lifestage = intermission[2] * 1 + 1;
        return gender + "-quote" + lifestage;
    }
    return "navigator";
}
SceneNavigator.prototype.getStartupScene = function getStartupScene() {return this._startupScene;};
